...
Density graph of size frequencies w/ mean length in a dashed line - seperated by year
```{r}
...
pcod_sl_full %>%
  filter(length < 150 & length_type == "SL",
         sample_year > 2005) %>%
  ggplot() +
  geom_density_ridges(aes(x = length, y = as.factor(oceanographic_domain), group = as.factor(oceanographic_domain), fill = as.factor(oceanographic_domain))) +
  geom_vline(data = pcod_means_oceano, aes(xintercept = mean_lng, color = as.factor(oceanographic_domain)), linetype = "dashed", linewidth = 1.5) +
  labs(x = "Length (mm)", y = "Density", fill = "Year") +
  theme_bw() +
  guides(color = "none")
```
...
pcod_sl_full %>%
  filter(length < 150 & length_type == "SL",
         sample_year > 2005) %>%
  ggplot() +
  geom_freqpoly(aes(x = length, group = as.factor(sample_year), color = as.factor(sample_year)), linewidth = 1.5) +
  geom_vline(data = pcod_means_oceano, aes(xintercept = mean_lng, color = as.factor(sample_year)), linetype = "dashed", linewidth = 1.5) +
  facet_wrap(~oceanographic_domain, ncol = 1) +
  labs(x = "Length (mm)", y = "Density", color = "Year") +
  theme_bw()
```
